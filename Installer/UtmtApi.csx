...
string EnvList(string name) => Environment.GetEnvironmentVariable(name) ?? "";

string overrides = EnvList("DRML_OVERRIDES");
string prefixes = EnvList("DRML_PREFIXES");
string postfixes = EnvList("DRML_POSTFIXES");
string sounds = EnvList("DRML_SOUNDS");
...
{
	throw new ScriptException("No patch lists provided.");
}
...
{
	if (!string.IsNullOrWhiteSpace(job.Item1))
	{
		foreach (string entry in job.Item1.Split(';'))
		{
			if (!string.IsNullOrWhiteSpace(entry))
			{
				...
				if (!File.Exists(path))
				{
					throw new ScriptException($"Patch file not found: {path}");
				}
...
{
	if (!string.IsNullOrWhiteSpace(soundEntry))
	{
...
if (applied == 0)
{
	throw new ScriptException("No patch entries matched this chapter.");
}
if (codePatches > 0)
{
	...
	{
		throw new ScriptException(result.PrintAllErrors(true));
	}
}
...
string ResolveSoundPath(string root, string soundFileName)
{
	...
	else
	{
		...
		{
			foreach (string candidate in Directory.EnumerateFiles(root, assetName + ".*"))
			{
				if (string.Equals(Path.GetExtension(candidate), ext, StringComparison.OrdinalIgnoreCase))
				{
					result = candidate;
					break;
				}
...
UndertaleSound FindSoundTemplate(string soundName)
{
	...
	foreach (var sound in Data.Sounds)
	{
		if (sound?.Name?.Content is string name)
		{
			any ??= sound;
...
{
	bool noEmbeddedAudio = sound.AudioFile is null;
	bool externalAudioId = sound.AudioID < 0;
	bool hasFilePath = !string.IsNullOrWhiteSpace(sound.File?.Content);
	return noEmbeddedAudio && externalAudioId && hasFilePath;
}
...
string GroupPath(int groupId)
{
	...
	{
		result = Paths.JoinVerifyWithinDirectory(gameDir, path);
	}
	else
	{
		result = Paths.JoinVerifyWithinDirectory(gameDir, $"audiogroup{groupId}.dat");
	}
	return result;
}
...
void SetSoundAudio(UndertaleSound sound, byte[] data, bool create)
{
	...
	else if (sound.GroupID == BuiltinGroup())
	{
		if (create)
		{
